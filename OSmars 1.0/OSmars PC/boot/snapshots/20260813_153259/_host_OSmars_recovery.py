...
''',
        ...
        if not (self.ROOT_DIR / "system" / "config.json").exists():
            default_config = {
                ...
                "mars_repo_url": "http://192.168.31.159:8765/",
...
"""
    ...
    def boot_rollback(self) -> bool:
        """Ręczny rollback z ostatniego snapshota (gdy status=failed)."""
        state = self._boot_read_state()
        snap_rel = state.get("snapshot")
        if not snap_rel:
            # najnowszy snapshot
            snaps = sorted((self.ROOT_DIR / "boot" / "snapshots").glob("*"))
            ...
            snap = self.ROOT_DIR / snap_rel
        if not snap.exists():
            print(self.colorize(f"Snapshot nie istnieje: {snap_rel}", "red"))
            return False
        print(self.colorize(f"Rollback z: {snap}", "cyan"))
        ok = self._boot_restore_snapshot(snap)
        if ok:
            # usuń zepsutą instrukcję opcjonalnie
            if self._boot_instrukcja_path().exists():
                self._boot_instrukcja_path().unlink()
            self._boot_write_state({
                "status": "idle",
    ...
    def _stage_py_transaction(self, py_path: Path, package_name: str) -> dict:
        ...
        instr = {
            ...
            "steps": [
                ...
                {"op": "write_ver", "component": pkg_id, "version": "0.0.0",
    ...
    def from_mars_uninstall(self, package_name: str):
        """Usuń zainstalowaną paczkę: apps/<id>, bin/<id>.py, ver/<id>.*"""
        ...
        if not pkg:
            ...
            ver = self.ROOT_DIR / "ver"
            installed = sorted(ver.glob("*.json")) if ver.is_dir() else []
            if installed:
                print("Zainstalowane:")
                for f in installed:
                    try:
                        d = json.loads(f.read_text(encoding="utf-8"))
                        print(f"  {d.get('id')}  v{d.get('version')}  ({d.get('name')})")
                    except Exception:
                        print(f"  {f.stem}")
            return
        ...
        removed = []
        targets = [
            self.ROOT_DIR / "apps" / pkg,
            self.ROOT_DIR / "bin" / f"{pkg}.py",
            self.ROOT_DIR / "ver" / f"{pkg}.json",
            self.ROOT_DIR / "ver" / f"{pkg}.txt",
            self.ROOT_DIR / "boot" / "pending" / pkg,
            self.ROOT_DIR / "boot" / "updates" / f"{pkg}.mars",
            self.ROOT_DIR / "boot" / "updates" / f"{pkg}.system",
            self.ROOT_DIR / "boot" / "updates" / f"{pkg}.bin",
        ]
        for path in targets:
            try:
                if path.is_file():
                ...
                print(self.colorize(f"  nie usunięto {path.name}: {e}", "yellow"))

        if removed:
            print(self.colorize(f"✅ Odinstalowano '{pkg}':", "green"))
            for r in removed:
                print(f"   - {r}")
        else:
            print(self.colorize(f"Nic nie znaleziono dla '{pkg}' (apps/bin/ver).", "yellow"))
    ...
    def _fetch_remote_packages(self) -> list:
        ...
        base = self.config.get("mars_repo_url", "http://192.168.31.159:8765/").rstrip("/")
    ...
    def from_mars_update(self, package_name: str = ""):
        """
        Porównaj lokalne ver/*.json z serwerem i zaktualizuj nowsze.
        from mars update [nazwa]
        """
        ...
        # jeśli podano nazwę — aktualizuj tylko ją (nawet bez lokalnego ver)
        if only:
            targets = [only]
        else:
            targets = sorted(local_ids)

        if not targets:
            print(self.colorize("Brak lokalnych wpisów w ver/ — zainstaluj coś albo podaj nazwę.", "yellow"))
            return
    ...
    def from_mars_install(self, package_name: str):
        ...
        base_url = self.config.get("mars_repo_url", "http://192.168.31.159:8765/").rstrip("/")
        stem = pkg
        lower = stem.lower()
